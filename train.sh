...
for minus_log_lr in $(seq 1 4); do
    for minus_log_wd in $(seq 1 4); do
        python3 train_on_snapshot.py --bs-train $bs_train --ds ml-10m/snapshots/snapshot-9 --gpu $gpu --logdir runs/cbmf/$minus_log_lr-$minus_log_wd --model "CollapsedBiasedMF(n_users, n_items, $d, r_mean)" --n-iters $n_iters --optim "Adam(model.parameters(), 1e-$minus_log_lr, weight_decay=1e-$minus_log_wd)" --p-train $p_train --p-val $p_val --semi &
        gpu=$((($gpu + 1) % $n_gpus))
        if [ $gpu -eq 0 ]; then
            wait
        fi
